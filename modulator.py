...
class BMod(A.Module):
	...
	async def motecmd(W,message):
		""" .mote <x><y><z> x - нижний пик y - верхний пик z - удерживание волны(3) """
		...
		P=G.repeat(G.arange(*J),O);S=G.sin(P);D=list(map(lambda x:x[0]*x[-1],zip(P,S)));K=1;await A.edit('<b>Работаю...</b>')
		for (T,L) in enumerate(I):
			U=D[T%E(D)];K+=1
			if K>E(D):K=0;D=D[::-1]
			V=R(L.frame_rate*U);H+=L._spawn(L.raw_data,overrides={'frame_rate':V})
		F=io.BytesIO();F.name='modulator.ogg';H.export(F,format='ogg',bitrate='64k',codec='libopus');F.seek(0);await A.client.send_file(A.to_id,F,reply_to=B.id,voice_note=True,duration=E(H)/1000);await A.delete()