...
class AudioEditorMod(loader.Module):
    ...
    async def basscmd(self, m):
        ...
        if not args: lvl = 2
            ...
            else: return await m.edit(f"[BassBoost] Укажи уровень от 2 до 100...")
...
async def go_out(m, audio, out, pref, title, fs=None):
    ...
    await m.client.send_file(m.to_id, o, reply_to=audio.reply.id, voice_note=audio.voice, attributes=[types.DocumentAttributeAudio(duration = fs if fs else audio.duration, title=title, performer="AudioEditor")] if not audio.voice else None)
    await m.delete()